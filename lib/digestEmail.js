...
export function buildWeeklyDigestBroadcast(roundup, configuration, { archiveUrl = "" } = {}) {
  ...
  return {
    ...
    subject,
    previewText,
    html: renderDigestHtml(roundup, configuration.publicAppUrl, { archiveUrl }),
    text: renderDigestText(roundup, configuration.publicAppUrl, { archiveUrl }),
    send: true
  };
}
...
export async function sendWeeklyDigestBroadcast(
  ...
  { illustrationResolver = addBirdIllustrations, archiveUrl = "" } = {}
) {
  const illustratedRoundup = await illustrationResolver(roundup, configuration.publicAppUrl);
  const broadcast = buildWeeklyDigestBroadcast(illustratedRoundup, configuration, { archiveUrl });
...
export function renderDigestHtml(roundup, appUrl = "https://flockline.app", { archiveUrl = "" } = {}) {
  ...
  const webLink = archiveUrl
    ? `<tr><td style="padding:6px 32px 0;font-size:12px;"><a href="${escapeHtml(archiveUrl)}" style="color:#4f5d50;">Read this issue on the web</a></td></tr>`
    : "";
      ...
      <tr><td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#fbfaf6;border:1px solid #d9d5ca;border-radius:18px;overflow:hidden;">
          ...
          <tr><td style="padding:24px 32px;background:#e9e6de;color:#696b64;font-size:12px;line-height:1.65;">
            ...
            You chose the ${escapeHtml(roundup.scopeLabel)} weekly edition. <a href="${UNSUBSCRIBE_URL}" style="color:#4f5d50;">Manage your regional editions or unsubscribe</a>.<br><br>
            Forwarded this email? <a href="${escapeHtml(`${safeAppUrl}/newsletter?src=forward`)}" style="color:#4f5d50;">Get your own free copy every Monday</a>.
          </td></tr>
        </table>
      </td></tr>
    </table>
...
export function renderDigestText(roundup, appUrl = "https://flockline.app", { archiveUrl = "" } = {}) {
  const safeAppUrl = String(appUrl).replace(/\/$/, "");
  const findings = Array.isArray(roundup.findings) ? roundup.findings : [];
  const sections = findings.map((finding, index) => {
    ...
    if (finding.image?.kind === "species-illustration") {
      lines.push("Flockline species illustration: not the reported individual.");
    }
  ...
  return [
    ...
    ...(archiveUrl ? ["", `Read this issue on the web: ${archiveUrl}`] : []),
    "",
    roundup.summary,
    "",
    ...
    `Forwarded this email? Get your own free copy every Monday: ${safeAppUrl}/newsletter?src=forward`
  ].join("\n");
}
...
function renderFindingHtml(finding, scopeId, appUrl) {
  ...
  const image = renderFindingImage(finding);
...
function renderFindingImage(finding) {
  const image = finding?.image;
  const imageUrl = safeRenderUrl(image?.url);
  if (!imageUrl || image?.kind !== "species-illustration") {
    return "";
  }

  return `<div style="margin:0 0 18px;background:#e9e6de;border:1px solid #ddd9cf;border-radius:14px;overflow:hidden;">
      <img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(image.alt || `Stylized field-guide illustration of ${finding.comName || finding.title}`)}" width="554" style="display:block;width:100%;height:auto;background:#e9e6de;border:0;" />
      <p style="margin:0;padding:8px 11px;color:#777970;font-size:10px;line-height:1.45;">Flockline species illustration · Not the reported individual.</p>
    </div>`;
}

function safeRenderUrl(value) {
  try {
...
function buildMapUrl(appUrl, speciesCode, scopeId) {
  ...
  url.searchParams.set("mode", "trail");
  url.searchParams.set("provisional", "1");
  url.searchParams.set("hotspots", "0");
  return url.toString();
}
