...
import { addBirdImages } from "./birdImages.js";
...
export async function sendWeeklyDigestBroadcast(
  ...
  { imageResolver = addBirdImages } = {}
) {
  const illustratedRoundup = await imageResolver(roundup);
  const broadcast = buildWeeklyDigestBroadcast(illustratedRoundup, configuration);
...
export function renderDigestText(roundup, appUrl = "https://flockline.app") {
  ...
  const sections = findings.map((finding, index) => {
    ...
    if (finding.image?.sourceUrl) {
      const credit = [finding.image.creator, finding.image.license].filter(Boolean).join(" · ");
      lines.push(`Photo: ${credit || finding.image.sourceName || "Wikimedia Commons"}`);
      lines.push(`Photo source: ${finding.image.sourceUrl}`);
    }
...
function renderFindingImage(finding) {
  ...
  const sourceUrl = safeRenderUrl(image?.sourceUrl);
  if (!imageUrl || !sourceUrl) {
    return "";
  }

  const credit = [image.creator, image.license].filter(Boolean).join(" · ");
  const licenseUrl = safeRenderUrl(image.licenseUrl);
  const license = licenseUrl && image.license
    ? `<a href="${escapeHtml(licenseUrl)}" style="color:#777970;">${escapeHtml(image.license)}</a>`
    : escapeHtml(image.license || "");
  const creditParts = [
    image.creator ? escapeHtml(image.creator) : "",
    license,
    `<a href="${escapeHtml(sourceUrl)}" style="color:#777970;">${escapeHtml(image.sourceName || "Wikimedia Commons")}</a>`
  ].filter(Boolean).join(" · ");

  return `<div style="margin:0 0 18px;background:#e9e6de;border:1px solid #ddd9cf;border-radius:14px;overflow:hidden;">
      <a href="${escapeHtml(sourceUrl)}" style="display:block;text-decoration:none;">
        <img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(image.alt || `${finding.comName || finding.title} bird photograph`)}" width="554" style="display:block;width:100%;height:260px;object-fit:cover;object-position:center;background:#e9e6de;border:0;" />
      </a>
      <p style="margin:0;padding:8px 11px;color:#777970;font-size:10px;line-height:1.45;">${creditParts || escapeHtml(credit)}</p>
    </div>`;
}
