...
describe("weekly digest email", () => {
  it("targets the matching topic and includes preference-safe email content", () => {
    ...
    expect(broadcast.text).toContain("mode=trail&provisional=1&hotspots=0");
  ...
  it("invites forwarded readers to subscribe and links the web issue", () => {
    const archiveUrl = "https://flockline.app/roundup/northeast/2026-08-17?src=email";
    const broadcast = buildWeeklyDigestBroadcast(roundup, configuration, { archiveUrl });

    expect(broadcast.html).toContain("Forwarded this email?");
    expect(broadcast.html).toContain("/newsletter?src=forward");
    expect(broadcast.html).toContain("Read this issue on the web");
    expect(broadcast.html).toContain(archiveUrl);
    expect(broadcast.text).toContain(`Read this issue on the web: ${archiveUrl}`);
    expect(broadcast.text).toContain("/newsletter?src=forward");

    // Without an archive URL the footer invite stays, the web link goes away.
    const plain = buildWeeklyDigestBroadcast(roundup, configuration);
    expect(plain.html).toContain("/newsletter?src=forward");
    expect(plain.html).not.toContain("Read this issue on the web");
  });
