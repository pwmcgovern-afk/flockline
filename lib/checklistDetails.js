// eBird's own codes: P21 is Stationary and P22 is Traveling. These two were
// inverted, so every record showed the wrong survey protocol and contradicted
// itself: "Stationary / 48.3 km" on a traveling count, and genuinely
// stationary checklists labelled Traveling with no distance at all.
...
export function normalizeChecklistDetails(raw, speciesCode) {
  ...
  return {
    ...
    checklistComments: cleanText(raw?.subComments ?? raw?.comments, 2400),
    observation: observation
      ? {
          speciesCode: cleanText(observation.speciesCode, 32) || speciesCode,
          count: observationCount(observation),
          // eBird sends this as `comments`, not `obsComments`, and media as a
          // single `mediaCounts: {P: 7}` object rather than three separate
          // counts. Reading the names we assumed meant the observer's own
          // notes and photo counts were silently dropped on every record: the
          // best detail in the whole dataset, never once displayed.
          comments: cleanText(observation.comments ?? observation.obsComments, 2400),
          breedingCode: cleanText(breeding?.auxCode || breeding?.value, 32),
          exoticCategory: cleanText(observation.exoticCategory, 64),
          media: {
            photos: mediaCount(observation.mediaCounts?.P ?? observation.photoCounts),
            audio: mediaCount(observation.mediaCounts?.A ?? observation.audioCounts),
            videos: mediaCount(observation.mediaCounts?.V ?? observation.videoCounts)
          }
        }
      : null
  };
