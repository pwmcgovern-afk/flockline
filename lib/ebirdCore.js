...
import { DEFAULT_REGION_ID, US_STATES, getCensusRegion, matchingCensusRegion } from "../shared/usGeography.js";
...
const MAX_REGION_STATES = Math.max(...["northeast", "midwest", "south", "west"].map(
  (regionId) => getCensusRegion(regionId)?.stateCodes.length ?? 1
));
...
function normalizeRegions(value, { fallbackOnInvalid = false } = {}) {
  ...
  const validCodes = new Set(US_STATES.map((state) => state.code));
  const codes = list
...
function describeRegionScope(regions) {
  const censusRegion = matchingCensusRegion(regions);
  if (censusRegion) {
    return `the ${censusRegion.name}`;
  }
...
export async function getSightings(query, apiKey = getApiKey()) {
  ...
  settled.forEach((result, index) => {
    const regionCode = regions[index];
    if (result.status === "fulfilled") {
      successfulRegions.push(regionCode);
...
export async function getInsights(query, apiKey = getApiKey()) {
  ...
  settled.forEach((result, index) => {
    if (result.status === "fulfilled") {
      successfulRegions.push(regions[index]);
      rows.push(...result.value);
    } else {
      failedRegions.push(regions[index]);
    }
  });
  if (!successfulRegions.length) {
    const error = new Error("eBird insights are temporarily unavailable for the selected states.");
    error.statusCode = 502;
    error.failedRegions = failedRegions;
    throw error;
  }
...
function buildDeterministicFindings(candidates, scopeLabel) {
...
function templateFinding(c, scopeLabel) {
  ...
  const meta = { speciesCode: c.speciesCode, comName: c.comName, locName: c.locName, region: c.regionName, obsDt: c.obsDt, subId: c.subId, lat: c.lat, lng: c.lng };
  if (c.kind === "wide") {
    return {
      kind: "wide",
...
function mergeLlmFindings(items, candidates, scopeLabel) {
