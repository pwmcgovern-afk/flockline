  ...
  try {
    for (let turn = 0; turn < CHAT_MAX_TURNS; turn += 1) {
      ...
      for (const block of response.content) {
        ...
        const result = await runChatTool(block.name, block.input, focusRegions, apiKey, speciesSeen);
        toolResults.push({
          type: "tool_result",
...
async function runChatTool(name, input, focusRegions, apiKey, speciesSeen) {
  if (!apiKey) {
  ...
  try {
    switch (name) {
      ...
      case "species_sightings":
        return await toolSpeciesSightings(input, focusRegions, apiKey, speciesSeen);
...
async function toolSpeciesSightings(input, focusRegions, apiKey, speciesSeen) {
