...
function templateFinding(c, scopeLabel) {
  ...
  if (c.kind === "wide") {
    return {
      ...
      detail: `Notable ${c.comName} reports reached ${c.stateCount} ${c.stateCount === 1 ? "state" : "states"} across ${scopeLabel} this week, including ${c.locName}, ${c.regionName}${when ? ` on ${when}` : ""}. The seven-day eBird feed included ${c.reportCount} ${c.reportCount === 1 ? "report" : "reports"}, with up to ${c.maxCount} ${c.maxCount === 1 ? "bird" : "birds"} in a single report.`,
      ...meta
    };
  }
  if (c.kind === "surge") {
    return {
      ...
      detail: `${c.reportCount} notable ${c.comName} ${c.reportCount === 1 ? "report formed" : "reports formed"} a concentrated run across ${scopeLabel} this week, including ${c.locName}, ${c.regionName}${when ? ` on ${when}` : ""}. The reports reached ${c.stateCount} ${c.stateCount === 1 ? "state" : "states"}, with up to ${c.maxCount} ${c.maxCount === 1 ? "bird" : "birds"} in a single report.`,
      ...meta
    };
  }
  ...
  return {
    ...
    detail: `${c.comName}${count} was reported at ${c.locName}, ${c.regionName}${when ? ` on ${when}` : ""}. Across ${scopeLabel}, the seven-day eBird feed included ${c.reportCount} notable ${c.reportCount === 1 ? "report" : "reports"} in ${c.stateCount} ${c.stateCount === 1 ? "state" : "states"}.`,
    ...meta
  };
}

function roundupContextSentence(candidate, scopeLabel) {
  return `Across ${scopeLabel}, the seven-day eBird feed included ${candidate.reportCount} notable ${candidate.reportCount === 1 ? "report" : "reports"} in ${candidate.stateCount} ${candidate.stateCount === 1 ? "state" : "states"}, with up to ${candidate.maxCount} ${candidate.maxCount === 1 ? "bird" : "birds"} in a single report.`;
}
...
function mergeLlmFindings(items, candidates, scopeLabel, limit = 4) {
  ...
  for (const item of items) {
    ...
    const modelDetail = String(item.detail || "").trim().slice(0, 500);
    const detail = modelDetail
      ? `${modelDetail}${hasTwoSentences(modelDetail) ? "" : ` ${roundupContextSentence(candidate, scopeLabel)}`}`
      : templateFinding(candidate, scopeLabel).detail;
...
Use only the supplied facts. Do not invent locations, dates, counts, species, causes, or migration explanations. Write each detail as two concise sentences, about 35 to 55 words total. Lead with the most useful sighting fact, then add context from the supplied report count, state count, maximum count, location, or date. Use periods and commas, never em dashes.
