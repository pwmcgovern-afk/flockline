...
import { normalizeSignupSource } from "./signupEvents.js";
...
export function createDigestConfirmationToken({ email, regionIds, src }, secret, now = Date.now()) {
  ...
  // The signup source rides inside the token so attribution survives the
  // double-opt-in hop: the contact only becomes real at confirm time.
  const normalizedSource = normalizeSignupSource(src);
  const body = Buffer.from(JSON.stringify({
    version: 1,
    email: normalizedEmail,
    regionIds: normalizedRegions,
    ...(normalizedSource ? { src: normalizedSource } : {}),
...
export function verifyDigestConfirmationToken(token, secret, now = Date.now()) {
  ...
  try {
    ...
    return {
      email,
      regionIds,
      // Optional field: tokens minted before source tracking existed stay valid.
      src: normalizeSignupSource(payload.src),
...
export async function sendDigestConfirmation({ email, regionIds, src }, configuration, resend = new Resend(configuration.apiKey)) {
